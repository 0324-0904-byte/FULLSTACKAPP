...
app.post('/login', (req, res) => {
    ...
    const authSql = "SELECT * FROM users WHERE name = ? AND password = ?";
...
app.get('/users', (req, res) => {
    db.query("SELECT * FROM users ORDER BY name ASC", (err, result) => {
...
app.post('/add-user', (req, res) => {
    ...
    db.query(checkSql, [name], (err, results) => {
        ...
        const sql = "INSERT INTO users (name, role, password, status) VALUES (?, ?, ?, 'Active')";
...
app.put('/update-user/:id', (req, res) => {
    ...
    const sql = "UPDATE users SET name = ?, role = ?, status = ? WHERE id = ?";
...
app.post('/folders', (req, res) => {
    const { name } = req.body;
    db.query("INSERT INTO folders (folder_name) VALUES (?)", [name], (err) => {
        if (err) return res.status(500).json({ success: false });
...
app.get('/documents', (req, res) => {
    ...
    let sql = "SELECT * FROM documents WHERE (title LIKE ? OR category LIKE ?)";
